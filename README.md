...
Опциональный AI Analyst подключается через OpenAI-compatible API (`AI_ANALYST_*`). Базовые правила дают первый сигнал, AI Analyst подтверждает/отклоняет его, затем risk manager принимает финальное разрешение. Без `AI_ANALYST_API_KEY` AI слой остаётся выключенным.
